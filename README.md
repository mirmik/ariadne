...
Удалённый shell выбирается из `SHELL`, затем из `PATH` как `sh`; при необходимости connector принимает `--shell /absolute/path`. Дочерний процесс получает небольшой allowlist окружения, но работает с правами того же OS-пользователя, что и connector.
