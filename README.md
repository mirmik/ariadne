...
Текущая версия wire-протокола — **2**. Relay и connector нужно обновлять вместе:
версия 1 отклоняется, поскольку не подписывает capabilities. Сохранённые identity,
registry и certificate pins совместимы; заново выполнять pairing не требуется.
...
CLI и MCP независимо ограничивают принимаемый файл 1 GiB. Другой предел в байтах
задаётся флагом запуска `--max-download-size`, например
`ari --max-download-size 2147483648 download phone /remote/file ./file`.
Для MCP тот же флаг передаётся `ariadne-mcp`. Ноль означает default, отрицательное
значение запрещено. Лимит отправляющего connector (`--max-file-size`) действует
отдельно и проверяется также при росте файла во время чтения.
...
`revoke` отзывает доступ, но не подтверждает остановку фоновых jobs. Если их нужно
остановить, отмените jobs перед отзывом; при недоступном узле требуется локальная
остановка процессов. Обрыв сети по-прежнему сохраняет фоновые задания.
...
Публичная регистрация ограничена 1024 неподтверждёнными неотозванными identities;
при заполнении новые identities отвергаются. Claim освобождает unclaimed-слот,
claimed/revoked записи автоматически не удаляются. Запись регистраций ограничена
4 в секунду с burst 16, а сериализованный registry — 16 MiB; отказ сохраняет
primary и backup целыми. Это ограничение ресурсов, а не закрытый admission:
публичные клиенты всё ещё способны занять квоту.
...
использует TLS 1.3, ALPN `ariadne/2`, отключённый 0-RTT и keepalive; при разрыве
